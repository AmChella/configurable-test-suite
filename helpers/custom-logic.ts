...
export const customLogicMap: Record<string, (page: any, step: any, context: Record<string, any>) => Promise<void>> = {
  ...
  const box = await page.evaluate(({ word, selector, nth }: { word: string; selector?: string; nth: number }) => {
      function findRangeForWord(root: Element | Document, word: string, nth: number): Range | null {
      ...
      const root: Element | Document = selector ? (document.querySelector(selector) as Element) || document : document;
    ...
    if (!box) {
      throw new Error(`Word '${word}' not found${selector ? ` within '${selector}'` : ''}.`);
    }
    ...
    if (wordwise) {
      ...
      const chord = plat === "darwin" ? "Alt+Shift+ArrowRight" : (plat === "win32" ? "Control+Shift+ArrowRight" : "Alt+Shift+ArrowRight");
